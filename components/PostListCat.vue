<template>
  <div class="w-full" v-if="posts">
    <div class="w-ful" v-if="!$helpers.isEmpty(posts)">
      <h2 class="md:text-2xl text-blue-600">{{ cat.title }}</h2>
      ...
      >
        ...
        >
          ...
          <h4 class="text-base">
            <router-link :to="`/${cat.slug}/article/${post.slug}`">{{
              post.title
            }}</router-link>
          </h4>
        </div>
